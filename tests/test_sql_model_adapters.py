...
from sqlalchemy import inspect, Integer, String, Boolean, DateTime, Float, Date, Time, LargeBinary
...
def test_sql_to_pydantic_all_types():
    ...
    # Check types
    assert fields["int_val"].annotation == int
    assert fields["float_val"].annotation == float
    assert fields["bool_val"].annotation == bool
    assert fields["str_val"].annotation == str
    assert fields["bytes_val"].annotation == bytes
    assert fields["datetime_val"].annotation == datetime
    assert fields["date_val"].annotation == date
    assert fields["time_val"].annotation == time
