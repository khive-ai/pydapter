...
class TestAsyncSQLAdapterErrorHandling:
    ...
    async def test_async_sql_connection_error(self, mock_sa, async_sql_sample):
        ...
        with patch("pydapter.extras.async_sql_.AsyncSQLAdapter.to_obj", side_effect=Exception("Connection error")):
            # Test to_obj with connection error
            with pytest.raises(Exception, match="Connection error"):
                await async_sql_sample.adapt_to(obj_key="async_sql", url="sqlite+aiosqlite:///test.db")
    ...
    async def test_async_sql_invalid_data(self, mock_sa, async_sql_sample):
        ...
        with patch("pydapter.extras.async_sql_.AsyncSQLAdapter.from_obj", side_effect=ValueError("Invalid data")):
