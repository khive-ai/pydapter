...
try:
    ...
    from pydapter.exceptions import ValidationError, ConfigurationError, TypeConversionError
    from pydapter.model_adapters.pg_vector_model import PGVectorModelAdapter
    from pydapter.model_adapters.sql_model import create_base, SQLModelAdapter
...
class EmbeddingWithRelationship(BaseModel):
    ...
    text: str
    embedding: list[float] = Field(..., json_schema_extra={"vector_dim": 1536})
    related_items: List[Dict[str, Any]] = Field(default_factory=list, json_schema_extra={
        "relationship": {
            "type": "one_to_many",
            "model": "RelatedItem",
...
def test_batch_insert_with_small_batch(mocker):
    ...
    items = [
        {"text": f"Item {i}", "embedding": [float(i)] * 1536} for i in range(1, 6)
    ]
