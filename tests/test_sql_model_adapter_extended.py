...
def test_python_type_for_basic():
    ...
    # Test each column
    assert SQLModelAdapter._python_type_for(int_column) == int
    assert SQLModelAdapter._python_type_for(bool_column) == bool
    assert SQLModelAdapter._python_type_for(date_column) == date
    assert SQLModelAdapter._python_type_for(datetime_column) == datetime
    assert SQLModelAdapter._python_type_for(time_column) == time
    assert SQLModelAdapter._python_type_for(binary_column) == bytes
