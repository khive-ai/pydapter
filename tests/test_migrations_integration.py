...
class TestMigrationsIntegration:
    ...
    def test_end_to_end_workflow(self, temp_dir, sqlite_engine, base_model, registry):
        ...
        adapter = adapter.init_migrations(migrations_dir, connection_string=connection_string, force_clean=True)
...
""")
        ...
        with sqlite_engine.connect() as conn:
            result = conn.execute(sa.text("SELECT name FROM sqlite_master WHERE type='table' AND name='users'"))
        ...
        with open(os.path.join(migrations_dir, "versions", f"{revision2}_add_email_column.py"), "w") as f:
            f.write(f"""
\"\"\"Add email column
...
""")
    ...
    def test_multiple_models(self, temp_dir, sqlite_engine, base_model, registry):
        ...
        # Create a migration script with table creation
        with open(os.path.join(migrations_dir, "versions", f"{revision1}_create_users_and_products_tables.py"), "w") as f:
            f.write(f"""
\"\"\"Create users and products tables
...
""")
        ...
        with sqlite_engine.connect() as conn:
            ...
            result = conn.execute(sa.text("SELECT name FROM sqlite_master WHERE type='table' AND name='products'"))
        ...
        # Create a migration script with column addition
        with open(os.path.join(migrations_dir, "versions", f"{revision2}_add_email_and_description_columns.py"), "w") as f:
            f.write(f"""
\"\"\"Add email and description columns
