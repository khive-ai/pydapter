...
class TestAsyncPostgresAdapterExtended:
    ...
    async def test_async_postgres_from_obj_with_dsn_conversion(self):
        ...
        with patch("pydapter.extras.async_postgres_.AsyncSQLAdapter.from_obj", AsyncMock()) as mock_from_obj:
            ...
            assert call_args["engine_url"] == "postgresql+asyncpg://user:pass@localhost/db"
    ...
    async def test_async_postgres_to_obj_with_dsn_conversion(self, async_postgres_sample):
        ...
        # Setup mocks for the parent class method
        with patch("pydapter.extras.async_postgres_.AsyncSQLAdapter.to_obj", AsyncMock()) as mock_to_obj:
            ...
            assert call_kwargs["engine_url"] == "postgresql+asyncpg://user:pass@localhost/db"
    ...
    async def test_async_postgres_to_obj_multiple_items(self):
        ...
        with patch("pydapter.extras.async_postgres_.AsyncSQLAdapter.to_obj", AsyncMock()) as mock_to_obj:
            ...
            assert call_args[1]["engine_url"] == "postgresql+asyncpg://user:pass@localhost/db"
