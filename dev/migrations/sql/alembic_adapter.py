    ...
    def create_migration(cls, message: str, autogenerate: bool = True, **kwargs) -> str:
        ...
        try:
            ...
            revision = command.revision(
                adapter.alembic_cfg, message=message, autogenerate=autogenerate
            )
    ...
    def get_current_revision(cls, **kwargs) -> Optional[str]:
        ...
        try:
            ...
            with adapter.engine.connect() as conn:
                context = environment.EnvironmentContext(adapter.alembic_cfg, script)
            ...
            if isinstance(exc, MigrationError):
                raise
            raise MigrationError(
                f"Failed to get current revision: {str(exc)}", original_error=str(exc)
            ) from exc
    ...
    def get_migration_history(cls, **kwargs) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(exc, MigrationError):
                raise
            raise MigrationError(
                f"Failed to get migration history: {str(exc)}", original_error=str(exc)
            ) from exc
...
""")
    ...
    def _create_migration_sync(
        ...
        if autogenerate and sync_conn is not None:
            ...
            script = ScriptDirectory.from_config(alembic_cfg)
            context = environment.EnvironmentContext(alembic_cfg, script)
            ...
            revision = command.revision(
                alembic_cfg, message=message, autogenerate=False
            )
            return revision.revision
    ...
    def _get_current_revision_sync(
        alembic_cfg: config.Config, sync_conn: Connection
    ) -> Optional[str]:
        """
