...
def test_migration_error_base_class():
    ...
    error = MigrationError(
        "Test migration error", adapter="test", directory="/tmp/migrations"
    )
...
def test_migration_init_error():
    ...
    error = MigrationInitError(
        "Failed to initialize migrations", directory="/tmp/migrations", adapter="test"
    )
...
def test_migration_upgrade_error():
    ...
    error = MigrationUpgradeError(
        "Failed to upgrade migrations", revision="head", adapter="test"
    )
...
def test_migration_downgrade_error():
    ...
    error = MigrationDowngradeError(
        "Failed to downgrade migrations", revision="abc123", adapter="test"
    )
...
def test_migration_not_found_error():
    ...
    error = MigrationNotFoundError(
        "Migration not found", revision="abc123", adapter="test"
    )
