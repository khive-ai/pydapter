...
class FieldTemplate:
    ...
    def _extract_pydantic_info(self) -> None:
        ...
        if get_origin(self.base_type) is Annotated:
            ...
            for arg in args[1:]:
                arg_str = str(arg)
                if any(constraint in arg_str for constraint in ['StringConstraints', 'Interval', 'Constraints']):
    ...
    def _merge_field_info(self, **overrides: Any) -> FieldInfo:
        ...
        if self._extracted_field_info:
            ...
            for attr in ["default", "default_factory", "title", "description", 
                        "examples", "exclude", "frozen", "alias"]:
        ...
        template_params = {k: v for k, v in template_params.items() if v is not Undefined}
    ...
    def create_field(self, name: str, **overrides: Any) -> Field:
        ...
        if self._extracted_type == self.base_type and get_origin(self.base_type) is Annotated:
        ...
        if hasattr(field_info, "default") and field_info.default not in (Undefined, PydanticUndefined):
            field_kwargs["default"] = field_info.default
        elif hasattr(field_info, "default_factory") and field_info.default_factory not in (None, Undefined):
